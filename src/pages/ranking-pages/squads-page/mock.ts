...
export const RankingSquadsPageContext = createContext<SquadsRankingProps | undefined>(undefined)

export const useRankingSquadsPageContext = () => {
  const context = useContext(RankingSquadsPageContext);
  if (context === undefined) {
    throw new Error('useRankingSquadsPageContext must be used within an RankingSquadsPageProvider');
  }
  return context;
}

export async function getRankingSquadsPage(): Promise<SquadsPageData> {
  await sleep(2000);
